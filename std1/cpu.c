...
{
    mprintf ("GROMRead: %04X : %02X\n",
            (int) gRom.addr,
            (int) gRom.rom.b[gRom.addr]);
...
execute (WORD data)
{
    ...
    case AMODE_SYM:
        ...
        //         (int) cpuRead (tms9900.pc), (int) sReg, (int) REG (sReg));
        sAddr = (WORD) (fetch() + (sReg == 0 ? 0 : REG (sReg)));
        break;

    case AMODE_INDIRINC:
        sAddr = cpuRead(REG(sReg));
        cpuWrite (AREG(sReg), (WORD) (REG(sReg) + (isByte ? 1 : 2)));
        break;
    }
    ...
    case AMODE_SYM:
        // printf ("SYM: @[%04X]+R%d (%04X) \n",
        //         (int) cpuRead (tms9900.pc), (int) dReg, (int) REG (dReg));
        dAddr = (WORD) (fetch() + (dReg == 0 ? 0 : REG (dReg)));
        break;

    case AMODE_INDIRINC:
        dAddr = cpuRead(AREG(dReg));
        cpuWrite (AREG(dReg), (WORD) (REG(dReg) + (isByte ? 1 : 2)));
        break;
    }
    ...
    if (isByte)
    {
        ...
        mprintf ("sData [%04X] = %02X, dData [%04X] = %02X\n",
                (int) sAddr, (int) sData >> 8,
                (int) dAddr, (int) dData >> 8);
    }
    else
    {
        sData = cpuRead (sAddr);
